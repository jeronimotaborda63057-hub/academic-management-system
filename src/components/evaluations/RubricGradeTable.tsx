    ...
    <section className="rounded-2xl border border-gray-200 bg-white">
        <div className="flex flex-wrap items-center justify-between gap-4 border-b border-gray-100 p-5">
            ...
            <div className="text-right">
                <p className="text-xs uppercase text-gray-400">Nota calculada</p>
        ...
        {loading ? (
            <div className="p-8 text-sm text-gray-500">Cargando informacion...</div>
        ) : criteria.length === 0 ? (
            <div className="p-8 text-sm text-gray-500">
                No hay criterios configurados para la rubrica de esta evaluacion.
            </div>
        ) : (
            <div className="overflow-x-auto">
                <table className="min-w-full text-sm">
                    <thead className="bg-gray-50 text-xs uppercase text-gray-500">
                        <tr>
                            <th className="px-5 py-3 text-left">Criterio</th>
                            <th className="px-5 py-3 text-left">Peso</th>
                            <th className="px-5 py-3 text-left">Nivel</th>
                            <th className="px-5 py-3 text-left">Puntaje</th>
                    ...
                    <tbody>
                        {criteria.map((criterion) => {
                            const selectedScale = scales.find(
                                (scale) => scale.id === draft[criterion.id]?.scaleId
                            );
                            const score =
                            ...
                            return (
                                <tr key={criterion.id} className="border-t border-gray-100">
                                    ...
                                    <td className="px-5 py-4 align-top">
                                        <select
                                            value={draft[criterion.id]?.scaleId ?? ""}
                                            onChange={(event) =>
                                        ...
                                        >
                                            ...
                                            {(scalesByCriterion[criterion.id] ?? []).map((scale) => (
                                                <option key={scale.id} value={scale.id}>
                                                    {scale.name} ({scale.value ?? 0})
                                                </option>
                                            ))}
                                        </select>
                                    </td>
