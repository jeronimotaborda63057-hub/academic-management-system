...
const toFormValues = (group: Partial<Group> | null): GroupFormValues => ({
    name: group?.name ?? "",
...
const Edit = () => {
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            try {
                const [group, semestersData, subjectsData, teachersData] = await Promise.all([
                ...
                toast.error("Error cargando el grupo.");
            }
        };
    ...
    const handleSubmit = async () => {
        ...
        try {
            const response = await groupService.update(id, values);
